...
import { NodeData, SkipList } from 'skip_list'
...
describe('SkipList', () => {
  ...
  describe('.getValue()', () => {
    ...
    it('should return the inserted value when present', () => {
      let s = new SkipList<string>()
        .insertAfter(null, 'key1', 'value1')
        .insertAfter('key1', 'key2', 'value2')
  ...
  describe('property-based tests', () => {
    function makeSkipListOps(size: number): SkipListOp[] {
      const numOps = jsc.random(0, Math.round(Math.log(size + 1) / Math.log(2)))
      const ops = new Array<SkipListOp>(numOps)
      const ids = []
    ...
    makeSkipListOps.map = (f: (t: SkipListOp[]) => undefined) => undefined
    makeSkipListOps.flatmap = (f: (t: SkipListOp[]) => undefined): undefined => undefined

    it('should behave like a JS array', () => {
      jsc.assert(
        jsc.forall(jsc.bless<SkipListOp[]>({ generator: makeSkipListOps }), function(ops) {
