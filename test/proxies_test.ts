...
describe('Automerge proxy API', () => {
  ...
  describe('list object', () => {
    ...
    describe('should support standard array read-only operations', () => {
      ...
      it('reduce()', () => {
        Automerge.change(root, (doc: ListTestDoc) => {
          ...
          assert.strictEqual(
            doc.numbers.reduce((sum: number, val: number, index: number) => (index % 2 === 0 ? sum + val : sum), 0),
            4
          )
        })
      })
      ...
      it('reduceRight()', () => {
        Automerge.change(root, (doc: ListTestDoc) => {
          ...
          assert.strictEqual(
            doc.numbers.reduceRight(
              (sum: number, val: number, index: number) => (index % 2 === 0 ? sum + val : sum),
              0
            ),
            4
          )
    ...
    describe('should support standard mutation methods', () => {
      ...
      it('push()', () => {
        root = Automerge.change(root, (doc: ListTestDoc) => {
          doc.noodles = []
        })
      ...
      it('splice()', () => {
        root = Automerge.change(root, (doc: ListTestDoc) => {
          assert.deepEqual(doc.numbers.splice(1), [2, 3])
        })
        assert.deepEqual(root.numbers, [1])
        root = Automerge.change(root, (doc: ListTestDoc) => {
          assert.deepEqual(doc.numbers.splice(0, 0, 999, 888, 777), [])
        })
        assert.deepEqual(root.numbers, [999, 888, 777, 1])
        root = Automerge.change(root, (doc: ListTestDoc) => {
          assert.deepEqual(doc.numbers.splice(1, 2, -1), [888, 777])
        })
