...
describe('Automerge.Frontend', () => {
  ...
  describe('backend concurrency', () => {
    ...
    it('should remove pending requests once handled', () => {
      ...
      const diffs1: Diff[] = [{ obj: ROOT_ID, type: 'map', action: 'set', key: 'blackbirds', value: 24 }]
  ...
  describe('applying patches', () => {
    ...
    it('should apply updates at different levels of the object tree', () => {
      ...
      assert.deepEqual(doc1, { counts: { magpies: 2 }, details: [{ species: 'magpie', family: 'corvidae' }]} as BirdSpeciesCounts)
      assert.deepEqual(doc2, { counts: { magpies: 3 }, details: [{ species: 'Eurasian magpie', family: 'corvidae' }]} as BirdSpeciesCounts)
    })
  })
})
