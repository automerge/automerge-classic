...
describe('Automerge.Backend', () => {
  describe('incremental diffs', () => {
    ...
    it('should make a conflict on assignment to the same key', () => {
      const change1: Change = {
        ...
        seq: 1,
        deps: {},
        ops: [{ action: 'set', obj: ROOT_ID, key: 'bird', value: 'magpie' }],
      }
      const change2: Change = {
        actor: 'actor2',
        seq: 1,
        deps: {},
        ops: [{ action: 'set', obj: ROOT_ID, key: 'bird', value: 'blackbird' }],
      }
      const s0 = Backend.init()
      const [s1, patch1] = Backend.applyChanges(s0, [change1])
      const [s2, patch2] = Backend.applyChanges(s1, [change2])
    ...
    it('should assign to keys in nested maps', () => {
      ...
      const change2: Change = {
        ...
        seq: 2,
        deps: {},
        ops: [{ action: 'set', obj: birds, key: 'sparrows', value: 15 }],
      }
      const s0 = Backend.init()
      const [s1, patch1] = Backend.applyChanges(s0, [change1])
      const [s2, patch2] = Backend.applyChanges(s1, [change2])
    ...
    it('should apply updates inside lists', () => {
      ...
      const change2: Change = {
        ...
        seq: 2,
        deps: {},
        ops: [{ action: 'set', obj: birds, key: `${actor}:1`, value: 'greenfinch' }],
      }
      const s0 = Backend.init()
      const [s1, patch1] = Backend.applyChanges(s0, [change1])
      const [s2, patch2] = Backend.applyChanges(s1, [change2])
    ...
    it('should delete list elements', () => {
      ...
      const change2: Change = {
        ...
        seq: 2,
        deps: {},
        ops: [{ action: 'del', obj: birds, key: `${actor}:1` }],
      }
      const s0 = Backend.init()
      const [s1, patch1] = Backend.applyChanges(s0, [change1])
      const [s2, patch2] = Backend.applyChanges(s1, [change2])
  ...
  describe('applyLocalChange()', () => {
    ...
    it('should throw an exception on duplicate requests', () => {
      ...
      const change2: Change = {
        ...
        seq: 2,
        deps: {},
        ops: [{ action: 'set', obj: ROOT_ID, key: 'bird', value: 'jay' }],
      }
      const s0 = Backend.init<any>()
      const [s1, patch1] = Backend.applyLocalChange(s0, change1)
  ...
  describe('getPatch()', () => {
    ...
    it('should handle increments for a key in a map', () => {
      ...
      const change1: Change = {
        ...
        seq: 1,
        deps: {},
        ops: [{ action: 'set', obj: ROOT_ID, key: 'counter', value: 1, datatype: 'counter' }],
      }
      const change2: Change = {
        actor,
        seq: 2,
        deps: {},
        ops: [{ action: 'inc', obj: ROOT_ID, key: 'counter', value: 2 }],
      }
      const s0 = Backend.init()
      const [s1, patch] = Backend.applyChanges(s0, [change1, change2])
      assert.deepEqual(Backend.getPatch(s1), {
    ...
    it('should create nested maps', () => {
      ...
      const change2: Change = {
        ...
        seq: 2,
        deps: {},
        ops: [{ action: 'del', obj: birds, key: 'wrens' }, { action: 'set', obj: birds, key: 'sparrows', value: 15 }],
      }
      const s0 = Backend.init()
      const [s1, patch] = Backend.applyChanges(s0, [change1, change2])
      assert.deepEqual(Backend.getPatch(s1), {
  ...
  describe('getChangesForActor()', () => {
    ...
    beforeEach(() => {
      oneDoc = Automerge.change(Automerge.init<DocBox>('actor1'), (doc: any) => (doc.document = 'watch me now'))
      twoDoc = Automerge.init('actor2')
      twoDoc = Automerge.change(twoDoc, (doc: any) => (doc.document = 'i can mash potato'))
      twoDoc = Automerge.change(twoDoc, (doc: any) => (doc.document = 'i can do the twist'))
