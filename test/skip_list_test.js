...
describe('SkipList', () => {
  describe('.indexOf()', () => {
    ...
    it('should return -1 for a nonexistent key', () => {
      let s = new SkipList<string>().insertAfter(null, 'a', 'a')
      assert.strictEqual(s.indexOf('b'), -1)
    })

    it('should return 0 for the first list element', () => {
      let s = new SkipList<string>()
        .insertAfter(null, 'b', 'b')
        .insertAfter('b', 'c', 'c')
  ...
  describe('.keyOf()', () => {
    ...
    it('should return null for an index past the end of the list', () => {
      let s = new SkipList<string>().insertAfter(null, 'a', 'a').insertAfter('a', 'b', 'b')
  ...
  describe('.getValue()', () => {
    ...
    it('should return the inserted value when present', () => {
      let s = new SkipList<string>().insertAfter(null, 'key1', 'value1').insertAfter('key1', 'key2', 'value2')
  ...
  describe('.setValue()', () => {
    it('should throw an exception when setting a nonexistent key', () => {
      let s = new SkipList<string>().insertAfter(null, 'key1', 'value1')
      assert.throws(() => {
  ...
  describe('.insertIndex()', () => {
    it('should insert the new key-value pair at the given index', () => {
      let s = new SkipList<string>().insertAfter(null, 'a', 'aa').insertAfter('a', 'c', 'cc')
    ...
    it('should insert at the head if the index is zero', () => {
      let s = new SkipList<string>().insertIndex(0, 'a', 'aa')
      assert.strictEqual(s.keyOf(0), 'a')
  ...
  describe('property-based tests', () => {
    function makeSkipListOps(size: number): Op[] {
      const numOps = jsc.random(0, Math.round(Math.log(size + 1) / Math.log(2)))
      const ops = new Array<Op>(numOps)
      const ids = []
    ...
    // keep jsc types happy
    makeSkipListOps.map = (f: (t: Op[]) => undefined) => undefined
    makeSkipListOps.flatmap = (f: (t: Op[]) => undefined): undefined => undefined

    it('should behave like a JS array', () => {
      jsc.assert(
        jsc.forall(jsc.bless<Op[]>({ generator: makeSkipListOps }), function(ops) {
          ...
          return (
            skipList.length === shadow.length &&
            shadow.every((id, index) => skipList.indexOf(id) === index && skipList.keyOf(index) === id)
          )
        }),
        { tests: 100, size: 50 }
      )
  ...
  describe('internal structure', () => {
    ...
    it('should keep track of skip distances', () => {
      ...
      assert.deepEqual(s._nodes.get('8'), {
        ...
        prevKey: ['7', '6', '4', null],
        prevCount: [1, 2, 4, 8],
        nextKey: [null, null, null, null],
        nextCount: [1, 1, 1, 1],
      })
    })

    it('should update preceding and succeeding nodes at the appropriate levels', () => {
      let s = new SkipList<string>(iter([5, 2, 1, 1, 1, 2, 5, 4]))
      s = s
        .insertAfter(null, '1', '1')
        .insertAfter('1', '2', '2')
        .insertAfter('2', '3', '3')
      ...
      assert.deepEqual(s._nodes.get('1'), {
        ...
        value: '1',
        level: 5,
        prevKey: [null, null, null, null, null],
        prevCount: [1, 1, 1, 1, 1],
        nextKey: ['2', '2', 'x', 'x', '7'],
        nextCount: [1, 1, 4, 4, 7],
      })
      ...
      assert.deepEqual(s._nodes.get('7'), {
        ...
        prevKey: ['6', '6', 'x', 'x', '1'],
        prevCount: [1, 1, 3, 3, 7],
        nextKey: [null, null, null, null, null],
        nextCount: [1, 1, 1, 1, 1],
      })
    })
