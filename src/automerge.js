...
function setField(state, objectId, key, value) {
  if (typeof key !== 'string') {
    throw new TypeError(
      'The key of a map entry must be a string, but ' + JSON.stringify(key) + ' is a ' + typeof key
    );
  }
  if (key === '') {
...
function makeChange(root, newState, message) {
  ...
  const change = fromJS({ actor, seq, deps, message }).set(
    'ops',
    newState.getIn(['opSet', 'local'])
  );
...
function checkTarget(funcName, target, needMutable) {
  if (
    ...
    throw new TypeError(
      'The first argument to Automerge.' +
        funcName +
        ' must be the object to modify, but you passed ' +
  ...
  if (needMutable && (!target._change || !target._change.mutable)) {
    throw new TypeError(
      'Automerge.' +
        funcName +
        ' requires a writable object as first argument, ' +
...
function assign(target, values) {
  ...
  if (!isObject(values))
    throw new TypeError(
      'The second argument to Automerge.assign must be an ' +
        'object, but you passed ' +
