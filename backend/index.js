...
function processChangeRequest(state, opSet, request, startOp) {
  ...
  for (let op of request.ops) {
    ...
    // For almost all objects, we treat the objectId generated by the frontend as temporary,
    // and map it into an operation ID. The only exception is a map inside a table (i.e. a table
    // row), for which we keep the frontend-generated objectId, because the frontend synchronously
    // returns that objectId to user code as primary key for the row.
    if (op.action.startsWith('make') && !(objType === 'makeTable' && op.action === 'makeMap')) {
