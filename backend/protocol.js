...
function generateSyncMessage(backend, peerState, changes) {
    ...
    const ourHeads = automerge_1.Backend.getHeads(backend), state = backend.state;
    ...
    const have = (!ourNeed.length) ? [sync_1.makeBloomFilter(state, sharedHeads)] : [];
    ...
    if (theirHave.length > 0) {
        ...
        if (!lastSync.every(hash => automerge_1.Backend.getChangeByHash(backend, hash))) {
    ...
    const heads = automerge_1.Backend.getHeads(backend);
    const syncMessage = {
        heads,
        have,
        need: ourNeed,
        changes: sync_1.getChangesToSend(state, theirHave, theirNeed)
    };
...
function receiveSyncMessage(backend, message, oldPeerState) {
    ...
    const beforeHeads = automerge_1.Backend.getHeads(backend);
    // when we receive a sync message, first we apply any changes they sent us
    if (changes.length) {
        unappliedChanges = [...unappliedChanges, ...changes];
        ourNeed = automerge_1.Backend.getMissingDeps(backend, unappliedChanges, heads);
        if (ourNeed.length === 0) {
            [backend, patch] = automerge_1.Backend.applyChanges(backend, unappliedChanges);
            unappliedChanges = [];
            sharedHeads = advanceHeads(beforeHeads, automerge_1.Backend.getHeads(backend), sharedHeads);
        }
    }
    const nextPeerState = {
        sharedHeads, ourNeed,
