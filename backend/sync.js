...
function generateSyncMessage(backend, syncState) {
  ...
  const { sharedHeads, lastSentHeads, theirHeads, theirNeed, theirHave, sentHashes } = syncState
  ...
  const hashesToSend = changesToSend.map(change => decodeChangeMeta(change, true).hash)
  syncState = Object.assign({}, syncState, {
    lastSentHeads: ourHeads,
    sentHashes: sentHashes.concat(hashesToSend),
  })
