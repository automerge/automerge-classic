...
function generateSyncMessage(backend, syncState) {
  ...
  const { sharedHeads, lastSentHeads, theirHeads, theirNeed, theirHave, sentChanges } = syncState
  ...
  if (sentChanges.length > 0 && changesToSend.length > 0) {
  ...
  syncState = {
    ...
    sentChanges: sentChanges.concat(changesToSend)
  }
